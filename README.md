...
Verity automates the validation and risk assessment of Software Bill of Materials (SBOM) files. It parses multi-format SBOMs, scores quality across seven weighted categories, checks compliance against major regulatory standards, queries live vulnerability databases, and produces structured reports. It replaces slow manual reviews with a repeatable, auditable process.

---

## Features

- **Multi-format parsing**: CycloneDX JSON/XML (1.0-1.7), SPDX JSON/YAML/tag-value/RDF (2.1, 2.2-2.3, 3.0); file format validated against per-spec allowed serializations
- **Quality scoring**: weighted 0-10 score across 7 categories (Structural, Identification, Provenance, Integrity, Licensing, Vulnerability and Traceability, Completeness) with letter grade A-F; base weight total 82; Component Security Health (weight 8) appended separately when vulnerability data is available
...
- **Risk scoring**: per-component scores based on missing fields, license type (AGPL/GPL/LGPL tiers), and CVE severity; document-level escalation when more than 30% of components are HIGH or above
...
|---|---|---|
| Structural Validity | 8 | Spec declaration, version support, per-spec file format check, JSON schema validity |
| Identification | 10 | Component names, versions, unique local IDs (bom-ref / SPDXID) |
| Provenance | 12 | Creation timestamp, authors, tool versions, namespace, supplier, lifecycle |
| Integrity | 15 | Checksums (any and strong SHA-256+), document-level signature (CycloneDX only; N/A for SPDX) |
| License Compliance | 15 | License presence, SPDX validity, declared licenses, deprecated/restrictive license detection |
| Vulnerability and Traceability | 10 | PURL and CPE syntax validity; both always applicable since absence is a scoring gap |
...
**File format validation** checks whether the serialization format matches the spec:
- CycloneDX: `json` or `xml`
- SPDX: `json`, `yaml`, `tv` (tag-value), or `rdf`

**Document signature** (`sbom_signature`) is marked N/A for SPDX documents because SPDX has no native signing mechanism. It is applicable only for CycloneDX.
...
|---|---|---|
| NTIA Minimum Elements (2021) | 7 required elements (9 profile features) | Per-component name, version, supplier, PURL, CPE; document author, timestamp, dependency relationships; machine-readable format check extended to yaml/tv/rdf |
| BSI TR-03183-2 v1.1 | SHALL + SHOULD tiers | CDX 1.4+ or SPDX 2.2+; creator contact, SHA-256 hash, license, dependency resolution; creator contact checks both supplier and manufacturer fields |
| BSI TR-03183-2 v2.0 | Adds: no vuln data, signature, BOM links | CDX 1.5+ or SPDX 2.2.1+; filename property, completeness declaration; BOM links check is CDX-only (N/A for SPDX) |
| BSI TR-03183-2 v2.1 | Strictest; CDX 1.6 or SPDX 3.0+ | Source URL, download URL, unique ID promoted to SHALL; component type property promoted to SHALL for CDX 1.6; SHA-512 on deployable artifact; declared licenses via acknowledgement=declared |
| FSCT v3 | Multi-level scoring (0/10/12/15) | Author 10=name/12=name+contact; checksum 10=any/12=strong; license 10=any/12=SPDX/15=SPDX+URL; lifecycle presence scores 15 (binary, not tiered); raw score exposed separately |
| OpenChain Telco v1.1 | SPDX only | 27 document + component checks; SPDXRef-DOCUMENT exact ID match; org/tool creator split (Organization: prefix required); supplier tiered 0/5/10 based on email presence; SHA-256 normalised; PURL, concluded/declared license, copyright text |
...
Verity bundles official JSON schemas for structural validation. Schemas are applied only to JSON-serialized SBOMs.
...
Documents using unknown or newer versions are not penalized (schema check is skipped rather than failed).
...
| `NVD_API_KEY` | (unset) | NVD API key for CVSS enrichment (optional) |
...
|   +-- app/
|   |   +-- core/
|   |   |   +-- parser.py          # Multi-format SBOM parser (CDX 1.0-1.7, SPDX 2.1-3.0)
...
+-- frontend/
    +-- src/
        ...
        +-- pages/                 # Dashboard, NewScan, ScanDetail, History, Analytics, Settings
